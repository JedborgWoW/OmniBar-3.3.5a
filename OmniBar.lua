...
function OmniBar:OnInitialize()
	...
	if addon.EventExists("GROUP_ROSTER_UPDATE") then
		self:RegisterEvent("GROUP_ROSTER_UPDATE", "GetSpecs")
	else
		-- 3.3.5a: GROUP_ROSTER_UPDATE does not exist; use the legacy events.
		self:RegisterEvent("PARTY_MEMBERS_CHANGED", "GetSpecs")
		self:RegisterEvent("RAID_ROSTER_UPDATE", "GetSpecs")
	end
	...
	for spellId, _ in pairs(self.cooldowns) do
		...
		-- 3.3.5a cannot render numeric fileID icons; prefer the client's string
		-- path (from GetSpellInfo) over any hardcoded fileID in the database.
		local existing = self.cooldowns[spellId].icon
		if type(existing) == "number" and type(icon) == "string" then existing = nil end
		self.cooldowns[spellId].icon = existing or icon
...
-- Lookup table mapping spell name -> spellID. Needed on Classic (the combat log
-- returns 0 for spellId) and on 3.3.5a (UNIT_SPELLCAST_SUCCEEDED carries no
-- spellID, only the spell name).
...
if WOW_PROJECT_ID == WOW_PROJECT_CLASSIC or WOW_PROJECT_ID == WOW_PROJECT_WRATH_CLASSIC then
	...
	for id, value in pairs(addon.Cooldowns) do
		if (not value.parent) then
			...
			if spellName then SPELL_ID_BY_NAME[spellName] = id end
		end
	end
end
...
function OmniBar:AddCustomSpells()
	...
	for k, v in pairs(self.db.global.cooldowns) do
		...
		if name then
			...
			addon.Cooldowns[k] = v
			-- Prefer the client's string icon path over a numeric fileID (3.3.5a).
			local existing = addon.Cooldowns[k].icon
			if type(existing) == "number" and type(icon) == "string" then existing = nil end
			addon.Cooldowns[k].icon = existing or icon
...
function OmniBar:Initialize(key, name)
	...
	if addon.EventExists("GROUP_ROSTER_UPDATE") then
		...
		-- 3.3.5a: GROUP_ROSTER_UPDATE does not exist; use the legacy events.
		f:RegisterEvent("PARTY_MEMBERS_CHANGED", "OnEvent")
		f:RegisterEvent("RAID_ROSTER_UPDATE", "OnEvent")
	end

	if WOW_PROJECT_ID ~= WOW_PROJECT_CLASSIC then
		f:RegisterEvent("PLAYER_FOCUS_CHANGED", "OnEvent")
		-- ARENA_OPPONENT_UPDATE does not exist on base 3.3.5a (some cores add it).
		if addon.EventExists("ARENA_OPPONENT_UPDATE") then
...
local function OmniBar_StartAnimation(self, icon)
	...
	-- 3.3.5a has no fromAlpha/toAlpha animation system, so drive the glow with a
	-- simple Lua alpha tween (see addon.AlphaPulse in Compat.lua).
	addon.AlphaPulse(icon.flash, 1, 0, 1)
	addon.AlphaPulse(icon.NewItemTexture, 1, 0, 1, function()
		OmniBar_UpdateBorder(self, icon)
	end)
end

local function OmniBar_StopAnimation(self, icon)
	addon.AlphaPulseStop(icon.flash)
	addon.AlphaPulseStop(icon.NewItemTexture)
...
function OmniBar:UNIT_SPELLCAST_SUCCEEDED(event, unit, arg2, arg3)
	-- 3.3.5a fires (unit, spellName, spellRank, lineID) with no spellID, while
	-- retail fires (unit, castGUID, spellID). Resolve a usable spellID for both.
	local spellID
	if type(arg3) == "number" and addon.Cooldowns[arg3] then
		spellID = arg3 -- retail: spellID is the third payload argument
	elseif SPELL_ID_BY_NAME then
		spellID = SPELL_ID_BY_NAME[arg2] -- 3.3.5a: spell name is the second payload argument
	end

	if (not spellID) or (not addon.Cooldowns[spellID]) then return end
...
function OmniBar:COMBAT_LOG_EVENT_UNFILTERED(_, ...)
	local event, sourceGUID, sourceName, sourceFlags, spellID, spellName
	if CombatLogGetCurrentEventInfo then
		-- Retail / modern classic: read from the accessor.
		local _, ev, _, sGUID, sName, sFlags, _, _, _, _, _, sid, sname = CombatLogGetCurrentEventInfo()
		event, sourceGUID, sourceName, sourceFlags, spellID, spellName = ev, sGUID, sName, sFlags, sid, sname
	else
		-- 3.3.5a payload (no hideCaster, no raid flags):
		-- timestamp, subevent, sourceGUID, sourceName, sourceFlags,
		-- destGUID, destName, destFlags, [spellID, spellName, spellSchool, ...]
		local _, ev, sGUID, sName, sFlags, _, _, _, sid, sname = ...
		event, sourceGUID, sourceName, sourceFlags, spellID, spellName = ev, sGUID, sName, sFlags, sid, sname
	end
	if (event == "SPELL_CAST_SUCCESS" or event == "SPELL_AURA_APPLIED") then
		if (spellID == 0 or not spellID) and SPELL_ID_BY_NAME then spellID = SPELL_ID_BY_NAME[spellName] end
