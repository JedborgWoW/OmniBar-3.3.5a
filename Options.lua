...
descriptions:SetScript("OnEvent", function(self, event, spellID, success)
	if success and GetSpellDescription then
		self[spellID] = GetSpellDescription(spellID);
	end
end);
-- SPELL_DATA_LOAD_RESULT does not exist on 3.3.5a; registering an unknown event
-- throws, so guard it. Spell descriptions simply stay blank on that client.
pcall(descriptions.RegisterEvent, descriptions, "SPELL_DATA_LOAD_RESULT");
		...
		for spellID, spell in pairs(addon.Cooldowns) do
			if spell.class and spell.class == CLASS_SORT_ORDER_WITH_GENERAL[i] then
				...
				if spellName then
					...
					if not descriptions[spellID] and C_Spell and C_Spell.RequestLoadSpellData then
